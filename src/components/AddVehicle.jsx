                    ...
                    <div className="rounded-3xl border border-slate-700/50 bg-slate-800/40 p-4 shadow-sm backdrop-blur sm:p-6">
                        <div className="space-y-4">
                            ...
                            <div className="rounded-2xl border border-slate-700/50 bg-slate-900/40 p-3">
                                <div className={errors?.vehicleImages ? "rounded-2xl ring-1 ring-rose-500/50" : ""}>
                                    <div className={errors?.vehicleImages ? "rounded-2xl bg-slate-900/60" : ""}>
                                        <Input
                                            type="file"
                                ...
                                {errors?.vehicleImages ? (
                                    <p className="mt-2 text-xs font-medium text-rose-300">{errors?.vehicleImages?.message || "Please upload at least one image."}</p>
                                ) : null}

                                {previews.length > 0 ? (
                                    <div className="mt-3">
                                        <div className="flex items-center justify-between">
                                            <p className="text-xs font-semibold tracking-wide text-slate-300">Selected</p>
                                            <p className="text-[11px] text-slate-500">{previews.length}/4</p>
                                        </div>
                                        <div className="mt-2 flex gap-2 overflow-x-auto pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                                            {previews.map((p, idx) => (
                                                <div key={`${p.url}-${idx}`} className="relative h-14 w-14 shrink-0 overflow-hidden rounded-2xl border border-white/10 bg-slate-900/40">
                                    ...
                                    <div className="mt-3 flex items-center gap-2 rounded-2xl border border-white/5 bg-slate-900/30 p-3">
                                        <div className="h-2.5 w-2.5 rounded-full bg-indigo-400" />
                                        <p className="text-xs text-slate-400">Add 1–4 clear photos. Front/side shots work best.</p>
                                    </div>
                                )}
